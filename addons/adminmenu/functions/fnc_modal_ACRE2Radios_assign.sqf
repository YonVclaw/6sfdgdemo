...
private _presetName = format ["6sfg_preset%1", _network];
...

{
    if (player canAdd _radio) then {
        ...
        systemChat format ["[6SFG] Added radio: %1", _radio];
    } else {
        if (getContainerMaxLoad uniform player > 0) then {
            (uniformContainer player) addItemCargoGlobal [_radio, 1];
            systemChat format ["[6SFG] Added radio (exceeds inventory capacity): %1", _radio];
        } else {
            systemChat format ["[6SFG] Couldn't add radio: %1", _radio];
        };
