...
if (isNil _utilityFunction) exitWith {
    systemChat format ["[6SFG Admin Menu] Modal utility with name '%1' requires undefined function '%2'", _utilityName, _utilityFunction];
};

GVAR(utilityData) = [];
private _exit = false;

if ((missionNamespace getVariable [QGVAR(selectedTab), -1]) isEqualTo IDC_6SFG_ADMINMENU_G_PMAN) then {
...
if (_exit) exitWith {
    systemChat "[6SFG Admin Menu] No valid player(s) selected for the action!";
};
