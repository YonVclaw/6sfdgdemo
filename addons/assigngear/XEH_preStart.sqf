...
uiNamespace setVariable ["6sfg_assignGear_validFaces",_faceClasses];

{
    ...
    if (count _weightedArray > 0) then {
        uiNamespace setVariable ["6sfg_assignGear_faceset_"+_name,_weightedArray];
    };
} forEach ("true" configClasses (configFile >> "6sfg_assignGear_facesets"));
