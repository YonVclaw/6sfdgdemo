...
/*
 * Name = 6SFG_chat_fnc_cmndWhisper
 * Author = Freddo
 *
 * Syntaxes:
...
if (_arg isEqualTo "") exitWith {
    systemChat "6SFG Error: No argument passed. Command usage: #whisper <player> <message>";
};
...
if (_message isEqualTo "") exitWith {
    systemChat "6SFG Error: No message passed. Command usage: #whisper <player> <message>";
};
...
if (!isNull _unit) then {
    ...
    systemChat format ["6SFG: Whisper sent to %1", name _unit];
} else {
    systemChat FORMAT_1("6SFG Error: No unit found containing %1, or more than one found.", str _message);
};
