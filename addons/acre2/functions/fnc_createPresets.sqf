...
private _calcChanFreq = {
    ...
    // e.g. [0,1,1] call 6sfg_acre2_calcChanFreq
...
6sfg_acre2_radioBasenameToSettingsIdx = {
    params ["_radio"];

    private _fndIdx = GVAR(radioCoreSettings) findIf {_radio in (_x select 0)};

    if (_fndIdx isEqualTo -1) then { 
        systemChat format["Warning - 6SFG ACRE2 Assign - GVAR(radioCoreSettings) missing radio: %1",_radio];
    };
    _fndIdx
};
...
//TODO Preserve shared channel indexes.
{
    ...
    //for "_i" from 1 to (count _radioPresetSetting)-1 do {
    {
        ...
        private _coreRadioIdx = [_radio] call 6sfg_acre2_radioBasenameToSettingsIdx;
    ...
    private _presetName = format["6sfg_preset%1",_radioPresetSettingIndex];
...
6sfg_acre2_networksCreated = true;