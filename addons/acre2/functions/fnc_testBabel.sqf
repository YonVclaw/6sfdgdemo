...
/* ----------------------------------------------------------------------------
Function: 6SFG_acre2_fnc_testBabel

Description:
    Checks if players have no Babel assigned to them.
    Used with 6SFG Autotest.
...
if (getmissionconfigvalue ["6SFG_AcreBabelEnabled",false]) then {
    ...
    private _babelArray = getMissionConfigValue ["6SFG_AcreBabelSettings", []];
...

    {
        ...
        if (_index == -1) then {
            ...
            private _groupCond = _grp getVariable ["6SFG_BabelLanguages", []];
