...
/*
 * Name: 6SFG_acre2_fnc_edenUnitToRadios
 * Author: Snippers
 *
 * Arguments:
 * 0: The Unit <OBJECT>
...
if (GVAR(giveMissingRadios)) then {
    ...
    if (_ourPresetIndex != -1) then {       
        private _unitVarChannelList = (_unit get3DENAttribute "6SFG_Channellist") param [0,[]];
        if (_unitVarChannelList isEqualType "") then { _unitVarChannelList = call compile _unitVarChannelList; };
        private _groupVarChannelList = (_unitGroup get3DENAttribute "6SFG_Channellist") param [0,[]];
        if (_groupVarchannelList isEqualType "") then { _groupVarchannelList = call compile _groupVarchannelList; };
        private _groupVarLeaderChannelList = (_unitGroup get3DENAttribute "6SFG_ChannellistLeader") param [0,[]];
