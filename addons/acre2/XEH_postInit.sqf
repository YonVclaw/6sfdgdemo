...
if (getMissionConfigValue ['6SFG_AcreBabelEnabled',false]) then {
    6SFG_BabelArray = getMissionConfigValue ["6SFG_AcreBabelSettings",[]];
    if (6SFG_BabelArray isEqualType "") then { 6SFG_BabelArray = call compile 6SFG_BabelArray; };
    {
        _x params ["_langName"];
...
/// Parse Radios
private _6sfgNetworkEnabled = (getMissionConfigValue ['6SFG_AcreNetworkEnabled',false]);
if (_6sfgNetworkEnabled) then {
    ...
    GVAR(radioAddActions) = getMissionConfigValue ['6SFG_AcreAddRadioActions', []];
    if (GVAR(radioAddActions) isEqualType "") then { GVAR(radioAddActions) = call compile GVAR(radioAddActions)};
    GVAR(networksWithRadioChannels) = getMissionConfigValue ['6SFG_AcreSettings', []];
...
if (hasInterface) then {
    [{
        ...
        params ["_6sfgNetworkEnabled"];
        if (_6sfgNetworkEnabled && isNil "6sfg_acre2_networksCreated") exitWith {}; //Ensure presets are created
        if (isNil QEGVAR(common,VarSync)) exitWith {}; // Ensure vars are recieved.

        [] call FUNC(clientInit);
        [_this select 1] call CBA_fnc_removePerFrameHandler;
    }, 1,_6sfgNetworkEnabled] call CBA_fnc_addPerFrameHandler;
