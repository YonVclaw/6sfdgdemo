...
// with uiNamespace do {     set3DENMissionAttributes [["6sfgMissionAcreAttributes","6SFG_AcreSettings",str RadioChannelArray]];    };
// with uiNamespace do {     set3DENMissionAttributes [["6sfgMissionAcreAttributes","6SFG_AcreSettings","[]"]];    };
...
fn_removeUnitFromChannel = {
    ...
    _list = (_unit get3DENAttribute "6SFG_Channellist") params ["_value"];
    if (_value isEqualType []) then {
        //Is default do nothing
        //_unit set3DENAttribute ["6SFG_Channellist",str []];
    } else {
        _value = call compile _value;
        _value = _value - [_channel];
        _unit set3DENAttribute ["6SFG_Channellist",str _value];
    };
};
...
fn_removeGroupFromChannel = {
    ...
    _list = (_group get3DENAttribute "6SFG_ChannellistLeader") params ["_value"];
    if (_value isEqualType []) then {
        //Is Default do nothing
        //_group set3DENAttribute ["6SFG_ChannellistLeader",str []];
    } else {
        _value = call compile _value;
        _value = _value - [_curSel];
        _group set3DENAttribute ["6SFG_ChannellistLeader",str _value];
    };

    
    _list = (_group get3DENAttribute "6SFG_Channellist") params ["_value"];
    if (_value isEqualType []) then {
        //is default do nothing
        //_group set3DENAttribute ["6SFG_Channellist",str []];
    } else {
        _value = call compile _value;
        _value = _value - [_curSel];
        _group set3DENAttribute ["6SFG_Channellist",str _value];
    };
    // do units
    {
...
fn_networkTreeHandleKey = {
    ...
    if (_thing isEqualType west) then {
        private _sideNum = _thing call 6SFG_common_fnc_sideToNum;
        {
            private _channel = _x;
        ...
        {
            set3DENAttributes [[units _x, "6SFG_Network", -1], [[_x],"6SFG_Network", -1]];
            set3DENAttributes [[units _x, "6SFG_Channellist", "[]"], [[_x],"6SFG_Channellist", "[]"], [[_x],"6SFG_ChannellistLeader", "[]"]];
    ...
    if (_thing isEqualType "") then {
        ...
        private _side = _sideNum call 6SFG_common_fnc_numToSide;
    ...
    if (_thing isEqualType grpNull) then {
        ...
        set3DENAttributes [[units _thing, "6SFG_Network", -1],
            [[_thing],"6SFG_Network",(_networkNumber-1)]];
        set3DENAttributes [[units _thing, "6SFG_Channellist", "[]"], [[_thing],"6SFG_Channellist", "[]"], [[_thing],"6SFG_Channellist", "[]"], [[_thing],"6SFG_ChannellistLeader", "[]"]];
    ...
    if (_thing isEqualType objNull) then {
        ...
        set3DENAttributes [[[_thing],"6SFG_Network", (_networkNumber-1)],
            [[group _thing],"6SFG_Network",-1]];
        set3DENAttributes [[[_thing], "6SFG_Channellist", "[]"], [[group _thing],"6SFG_Channellist", "[]"], [[group _thing],"6SFG_ChannellistLeader", "[]"]];
...
switch _mode do {
    case "onLoad": {
        ...
        RadioChannelArray = ("6SFG_MissionAcre2Attributes" get3DENMissionAttribute "6SFG_AcreSettings");
        ...
        if (isNil "RadioChannelArray") then {
            ...
            RadioChannelArray = [];
            //RadioChannelArray = call compile ("6sfgMissionAcreAttributes" get3DENMissionAttribute "6SFG_AcreSettings");
        };
        ...
        //Deserialize: Convert nums to side from
        {
            ...
            _network params ["_conditions", "_channels"];
            {
                if (_x isEqualType 0) then {
                    _conditions set [_forEachIndex,(_x call 6SFG_common_fnc_numToSide)];
                };
            } forEach (_conditions);
            {
                ...
                {
                    if (_x isEqualType 0) then {
                        _condition set [_forEachIndex,(_x call 6SFG_common_fnc_numToSide)];
                    };
                } forEach (_condition);
            } forEach (_channels);
        } forEach RadioChannelArray;
        ...
        private _array = + (uiNamespace getVariable "RadioChannelArray");
        {
            ...
            _network params ["_conditions", "_channels"];
            {
                if (_x isEqualType east) then {
                    _conditions set [_forEachIndex, (_x call 6SFG_common_fnc_sideToNum)];
                };
            } forEach (_conditions);
            {
                private _condition = _x select 3; // condition
                {
                    if (_x isEqualType east) then {
                        _condition set [_forEachIndex, (_x call 6SFG_common_fnc_sideToNum)];
                    };
                } forEach (_condition);
            } forEach (_channels);
        } forEach _array;
    ...
    case "refreshChannelTree": {
        ...
        fn_channelTreeProcessUnit = {
            ...
            private _color = (side _unit) call 6SFG_common_fnc_sideToColor;
            ...
            if (!_giveRadio) then {
                private _unitChanList = (_unit get3DENAttribute "6SFG_Channellist") select 0;
                if (_unitChanList isEqualType "") then {
                    _unitChanList = call compile _unitChanList;
                };
                //private _unitChanList = (call compile ((_unit get3DENAttribute "6SFG_Channellist") select 0));
            ...
            if (_giveRadio) then {
                _ctrlTree tvSetPictureRight [_location, "z\6sfg\addons\acre2\ui\RadioIcon_Radio_Small.paa"];
                _returnCode = 0;
            } else {
                _ctrlTree tvSetPictureRight [_location, "z\6sfg\addons\acre2\ui\RadioIcon_None_Small.paa"];
            };
        ...
        fn_channelTreeProcessGroup = {
            ...
            private _color = _side call 6SFG_common_fnc_sideToColor;
            ...
            if (!_giveRadio) then {
                private _grpChanList = (_group get3DENAttribute "6SFG_Channellist") select 0;
                ...
                if (RadioCurrentNetworkChannel in _grpChanList) then {
                    ...
                    _grpChanList = (_group get3DENAttribute "6SFG_ChannellistLeader") select 0;
                    if (_grpChanList isEqualType "") then {
                        _grpChanList = call compile _grpChanList;
                    };
                    if (RadioCurrentNetworkChannel in _grpChanList) then {
            ...
            if (_giveRadio) then {
                ...
                if (_giveLeader) then {
                    _ctrlTree tvSetPictureRight [_location, "z\6sfg\addons\acre2\ui\RadioIcon_Leader_Small.paa"];
                    _returnCode = 2;
                } else {
                    if (_gaveSomeoneARadio) then {
                        _ctrlTree tvSetPictureRight [_location, "z\6sfg\addons\acre2\ui\RadioIcon_Partial_Small.paa"];
        ...
        private _sides = [];
        {
            ...
            if (_x isEqualType east) then {    
                ...
                if (_sides pushBackUnique _side != -1) then {
                    ...
                    private _location = [(_ctrlTree tvAdd [[], _side call 6SFG_common_fnc_sideToString])];
                    
                    _ctrlTree tvSetPicture [_location, _side call 6SFG_common_fnc_sideToTexture];
        ...
        
        {
            ...
            if (_sides find (side _group) == -1) then {
                if (_sides find (toLower faction (leader _group)) == -1) then {
                    if (((_group get3DENAttribute "6SFG_Network") select 0) == RadioCurrentNetwork) then {
                        [_ctrlTree, [], false, _x] call fn_channelTreeProcessGroup;
                    } else {
                        {
                            if (((_x get3DENAttribute "6SFG_Network") select 0) == RadioCurrentNetwork) then {
    ...
    case "refreshNetworkTree": {
        ...
        _factions = [];
        {
            ...
            //Find Side
            if (_sideIdx == -1) then {
                private _sideText = _side call 6SFG_common_fnc_sideToString; 
                private _sideIcon = _side call 6SFG_common_fnc_sideToTexture; 
                ...
                if (_networkNumber != -1) then {
                    _ctrlTree tvSetPictureRight [[_sideIdx], (_networkNumber+1) call 6SFG_common_fnc_numToTexture];
                } else {
            ...
            if (_factionIdx == -1) then {
                ...
                if (_networkNumber != -1) then {
                    _ctrlTree tvSetPictureRight [[_sideIdx, _factionIdx], (_networkNumber+1) call 6SFG_common_fnc_numToTexture];
                } else {
            ...
            if (_networkNumber != -1) then {
                _ctrlTree tvSetPictureRight [[_sideIdx, _factionIdx, _grpIdx], (_networkNumber+1) call 6SFG_common_fnc_numToTexture];
            } else {
                _networkNumber = (_group get3DENAttribute "6SFG_Network") select 0;
                if (_networkNumber != -1) then {
                    _ctrlTree tvSetPictureRight [[_sideIdx, _factionIdx, _grpIdx], (_networkNumber+1) call 6SFG_common_fnc_numToTexture];
                };
            };
            ...
            private _preUnitNetworkNumber = _networkNumber;
            {
                ...
                if (_networkNumber != -1) then {
                    _ctrlTree tvSetPictureRight [[_sideIdx, _factionIdx, _grpIdx, _unitIdx], (_networkNumber+1) call 6SFG_common_fnc_numToTexture];
                } else {
                    _networkNumber = (_x get3DENAttribute "6SFG_Network") select 0;
                    if (_networkNumber != -1) then {
                        _ctrlTree tvSetPictureRight [[_sideIdx, _factionIdx, _grpIdx, _unitIdx], (_networkNumber+1) call 6SFG_common_fnc_numToTexture];
                    };
                };
    ...
    case "channelDelClick": {
        with uiNamespace do {
            ...
            private _units = allUnits select {([_x] call 6SFG_acre2_fnc_unitToPreset) == RadioCurrentNetwork};
            private _groups = []; {_groups pushBackUnique (group _x);} forEach _units;
            {
                private _entity = _x;
                private _list = (_entity get3DENAttribute "6SFG_Channellist") params [["_value",[]]];
                if (_value isEqualType "") then {
                    _value = call compile _value;
                };
                ...
                _entity set3DENAttribute ["6SFG_Channellist",str _value];
            } forEach (_units + _groups);
            // Cycle through groups to remove leader.
            {
                private _entity = _x;
                private _list = (_entity get3DENAttribute "6SFG_ChannellistLeader") params [["_value",[]]];
                if (_value isEqualType "") then {
                    _value = call compile _value;
                };
                _value = _value - [_curSel]; // Remove current channel.
                ...
                _entity set3DENAttribute ["6SFG_ChannellistLeader",str _value];
            } forEach _groups;
        ...
        with uiNamespace do {
            ...
            if (_entity isEqualType east or _entity isEqualType "") then {
                ...
                if (_entity isEqualType grpNull or _entity isEqualType objNull) then {
                    _list = (_entity get3DENAttribute "6SFG_Channellist") params ["_value"];
                    if (_value isEqualType []) then {
                        _entity set3DENAttribute ["6SFG_Channellist",str [_curSel]];
                    } else {
                        _value = call compile _value;
    ...
    case "channelTreeLeader": {
        with uiNamespace do {
            ...
            if (_entity isEqualType grpNull) then {
                _list = (_entity get3DENAttribute "6SFG_ChannellistLeader") params ["_value"];
                if (_value isEqualType []) then {
                    _entity set3DENAttribute ["6SFG_ChannellistLeader",str [_curSel]];
                } else {
                    _value = call compile _value;
                    _value pushBackUnique _curSel;
                    _entity set3DENAttribute ["6SFG_ChannellistLeader",str _value];
        ...
        with uiNamespace do {
            ...
            if (_entity isEqualType east or _entity isEqualType "") then {
                ...
                if (_entity isEqualType east) then {
                    ...
                    private _sideNum = _entity call 6SFG_common_fnc_sideToNum;
                    {
                        if (_x isEqualType "") then {
                            if (_sideNum == getNumber (configfile >> "CfgFactionClasses" >> _x >> "side")) then {
    ...
    case "save": {
        ...
        set3DENMissionAttributes [["6sfgMissionAcreAttributes", "6SFG_AcreSettings", _string]];
    };
};
